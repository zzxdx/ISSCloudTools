...
-(BOOL)imageIsExicts:(NSString *)requestUrl{
    NSURL * filePath = [self getDownloadDataFromCacheWithRequestUrl:requestUrl];
    NSLog(@"%@",filePath);
    NSFileManager *fileManager = [NSFileManager defaultManager];
    if ([fileManager fileExistsAtPath:[NSString stringWithFormat:@"%@",filePath]]) {
        NSLog(@"222文件abc.doc存在");
        return YES;
    }
    else {
        NSLog(@"222文件abc.doc不存在");
        return NO;
    }
